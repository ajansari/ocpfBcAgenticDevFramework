...
| Network access | Reads public files from GitHub (`raw.githubusercontent.com`, `api.github.com`): the runbooks, the Standards Guide, BCQuality, and the OCPF patterns library. Reads Microsoft Learn and AL Guidelines documentation. Microsoft's AL tools reach the user's own Business Central environment only when the user signs in to publish or download symbols. |
