...
| Files it writes | Only in the user's project, and only as the routine describes: `CLAUDE.md` and/or `.github/copilot-instructions.md`, the runbook changelog, the fetched `standardsGuide/` and `opsGuide/` copies, `.ocpf/framework.json`, the project's design documents, AL code, and, after approval, `scripts/`, an `al` entry in `.mcp.json`, the user's notification choice (`.ocpf/notifications.json`), and Claude Code notification settings (`.claude/settings.local.json`). Outside the project, with approval and only for the kinds the user chose: `remoteControlAtStartup` in `~/.claude/settings.json`, VS Code user settings for Copilot Chat's notifications and sounds, and `~/.copilot/hooks/ocpf-notify.json` for Copilot CLI sounds. It never overwrites an existing `CLAUDE.md` or Copilot instructions file. |
