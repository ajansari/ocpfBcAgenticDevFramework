...
| Scripts | `al-mcp-setup/scripts/`: `al-mcp.sh`, `al-mcp.cmd`, `al-mcp-resolve.ps1` (start Microsoft's AL MCP Server), `al-mcp-call.sh`, `al-mcp-call.ps1` (run one AL MCP Server tool), and `al-analyze.sh`, `al-analyze.cmd`, `al-analyze-resolve.ps1` (compile with Microsoft's code analyzers); `notifications/scripts/`: `ocpf-notify.sh`, `ocpf-notify.ps1` (play a short system sound when it's the user's turn). They're copied into the user's project, through the AI tool's own permission prompts, and use the locally installed AL Language extension. They install nothing. |
