...
| `notifications` skill | Notifies you whenever the agent finishes a turn, asks a question, or waits for an approval, using each AI tool's own notifications: VS Code's for Copilot Chat, Copilot CLI's built-in ones, and Claude app push plus a sound for Claude Code. The runbook sets this up at its first step; the skill adds it to older projects. |
