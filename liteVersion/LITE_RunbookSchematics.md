...
flowchart TD
    ...
    subgraph BUILD["PHASE: BUILD"]
        direction TB
        S3["Step 3<br/>Plan & Scaffold"] --> G_S3{{"Batch order agreed;<br/>scaffold structurally<br/>complete (not compiled)"}}
    ...
    subgraph PROVE["PHASE: PROVE"]
        ...
        S7 --> G_S7{{"Green tests pass,<br/>red tests fail gracefully →<br/>same package ships to Production"}}
    end
...
flowchart TD
    ...
    subgraph S4["STEP 4 — Generate the Code (repeats per object)"]
        direction TB
        B1["Pause for human approval,<br/>then generate this object's<br/>AL file from the Standards §1.3<br/>template"]
    ...
    subgraph S5["STEP 5 — Compile, Package, Test & Iterate"]
        ...
        Diag --> Fix["Fix root cause, regenerate<br/>files, log ChangeLog.md,<br/>update Design Doc"]
...
flowchart TD
    ...
    GapFill -- "Yes" --> GapCycle["Step 5 cycle again —<br/>fix, compile, package,<br/>redeploy, retest"]
    GapCycle --> Gate1
    ...
    subgraph S7["STEP 7 — Release for Testing"]
        direction LR
        In2[/"Inputs:<br/>Latest package,<br/>TestScript.md, Docs.md"/] --> Act2["Actions:<br/>Real users run TestScript.md<br/>by hand on the sandbox —<br/>green + red team, permission-set<br/>verification; record findings in<br/>ChangeLog.md (implement / defer / reject)"] --> Out2[/"Output:<br/>ChangeLog.md updated with<br/>every test finding"/]
    end
...
flowchart LR
    ...
    class DEFINE,DESIGN,BUILD,PROVE phase;
    class Change,Pack,Std,Hygiene,MCP,BCQ,Pat,Perm discipline;
```

---

*Generated from `LITE_BC_App_Build_Routine_Agent.md` v1.2.0.0 (September 13, 2026 — first release
of this schematics file, alongside `ProjectParameters.md` as Step 1's own persisted artifact; see
...
hand-edit a diagram without checking it still parses.*
